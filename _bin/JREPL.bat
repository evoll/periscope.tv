...
::JREPL.cmd version 3.6 by Dave Benham
::
::  Release History:
...
:::
:::  JREPL.cmd was written by Dave Benham, and originally posted at
...
if .%2 equ . (
  if "%~1" equ "/?" (
    ...
    for /f "tokens=* delims=:" %%A in ('findstr "^::JREPL\.cmd" "%~f0"') do @echo(%%A
    exit /b 0
  ) else (
